...
import { contactInfo, faqs, socialLinks } from "../../data/contact";

export function ContactInfoPanel() {
  return (
    <aside className="space-y-6 lg:col-span-2">
      {contactInfo.map(({ icon: Icon, label, value, href, color, background }) => (
        <a
          key={label}
          href={href}
          target={href.startsWith("http") ? "_blank" : undefined}
          rel={href.startsWith("http") ? "noreferrer" : undefined}
          className={`flex items-center gap-4 rounded-2xl border p-5 transition-transform hover:scale-[1.02] ${background}`}
        >
        ...
        <div className="flex gap-3">
          {socialLinks.map(({ icon: Icon, label, href }) => (
            <a
              ...
              target={href.startsWith("http") ? "_blank" : undefined}
              rel={href.startsWith("http") ? "noreferrer" : undefined}
              className="flex flex-1 items-center justify-center rounded-xl border border-white/10 bg-white/5 py-2.5 text-gray-400 transition-all hover:border-violet-500/30 hover:text-violet-300"
            >
              <Icon size={16} />
            </a>
          ))}
