...
const categoryColors: Record<Project["category"], string> = {
  "Full-Stack": "bg-violet-500/10 text-violet-300 border-violet-500/20",
  Frontend: "bg-emerald-500/10 text-emerald-300 border-emerald-500/20",
  Mobile: "bg-fuchsia-500/10 text-fuchsia-300 border-fuchsia-500/20",
};
...
export function ProjectCard({ project, compact = false }: ProjectCardProps) {
  return (
    <article className="group overflow-hidden rounded-2xl border border-white/5 bg-white/[0.02] transition-all duration-300 hover:-translate-y-1 hover:border-violet-500/20">
      <div className={cn("relative overflow-hidden", compact ? "h-48" : "h-52")}>
        ...
        <div className="absolute right-3 top-3 flex gap-2 opacity-0 transition-all duration-200 group-hover:translate-y-0 group-hover:opacity-100">
          <a
            ...
            className="flex h-8 w-8 items-center justify-center rounded-full bg-black/50 text-white backdrop-blur-sm transition-colors hover:bg-violet-600"
          >
            <Github size={13} />
          </a>
          <a
